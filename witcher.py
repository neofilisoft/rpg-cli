...
class Witcher:
    def __init__(self, name, school):
        ...
        self.exp = 0
        self.gold = 100
        self.inventory = ["Witcher Silver Sword", "Bread", "Dwarven Spirit"] # เริ่มต้นมีเหล้าไว้ปรุงยา
        self.active_quests = []
        ...
        if school == "Wolf":
            ...
            self.base_dmg = 7
            self.crit_chance = 10
            self.inventory.append("Thunderbolt") # เริ่มพร้อมยา
    ...
    while player.hp > 0 and enemy_hp > 0:
        ...
        if choice == "1":
            if roll_dice(100) <= player.crit_chance:
                dmg_dealt = player.base_dmg * 2
                print(f"{Colors.YELLOW}CRITICAL HIT!{Colors.END}")
            else:
                dmg_dealt = player.base_dmg + roll_dice(3)
        
        elif choice == "2":
            if roll_dice(100) > 40: # Hit chance
                dmg_dealt = player.base_dmg + roll_dice(8) + 2
                print("ฟันรุนแรง!")
            else:
            ...
            if effect == "stun": enemy_stunned = True # Aard stun
            if effect == "hypnotize": # Axii effect
                enemy_stunned = True
            ...
            if monster['weakness'] == "Axii" and s=="5": 
                ...
                enemy_stunned = True # Guarantee stun
        
        elif choice == "4":
            print(f"Inventory: {player.inventory}")
            use = input("พิมพ์ชื่อไอเทมที่จะใช้: ")
            if use in player.inventory:
                if use == "Swallow":
                    player.heal(30)
                elif use == "Thunderbolt":
                    print("พลังโจมตีเพิ่มขึ้นชั่วคราว!")
                    dmg_dealt += 10 # ใช้เทิร์นนี้เลย
...
def game_loop():
    ...
    while True:
        ...
        if act == "1":
            ...
            # ถ้ามีเควสต์ ให้เพิ่มโอกาสเจอมอนสเตอร์ตัวนั้น
            possible = list(monsters.keys())
            if player.active_quests:
                possible.extend(player.active_quests * 3) # เพิ่มน้ำหนักสุ่ม
